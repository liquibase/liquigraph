...
public class TestNeo4jContainer implements AutoCloseable {
    ...
    private static Neo4jContainer<?> createContainer(String coordinates, String adminPassword) {
        ...
        return new Neo4jContainer<>(coordinates)
            ...
            .withNeo4jConfig("dbms.unmanaged_extension_classes", String.format("%s=/ext", ClearDatabaseExtension.class.getPackage().getName()))
            // the following workaround is needed only for 3.0.0
            .withEnv("NEO4J_dbms_unmanagedExtensionClasses", String.format("%s=/ext", ClearDatabaseExtension.class.getPackage().getName()))
